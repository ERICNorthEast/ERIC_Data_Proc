...
getDurhamConfig <- function(bat_sp,plant_sp,Other_LA_Cols){
  ...
  Durham_split <- tibble::tribble(~SheetLabel,~FilterString,
                          ...
                          "Plant_LowerPlantProtected_data",paste0("SLA_data$`All UK Legally Protected (W&C Act, Badgers and Cons Regs) - Taxon Designated`=='Yes' & stringr::str_detect(tolower(SLA_data$`Taxon group`),'",paste(unlist(plant_sp),collapse = '|'),"')" ),
                          ...
                          "DBAP_Plants_LowerPlants_data",paste0("SLA_data$`All UK Legally Protected (W&C Act, Badgers and Cons Regs) - Taxon Designated`=='No' & stringr::str_detect(SLA_data$`North East LBAP - Short Names`,'Durham BAP') & stringr::str_detect(tolower(SLA_data$`Taxon group`),'",paste(unlist(plant_sp),collapse = '|'),"')"),
